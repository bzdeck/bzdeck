...
/*
 * Get attachment data by ID. If it's not in the cache, retrieve it from Bugzilla.
 *
 * [argument] id (Number) the attachment id
 * [argument] format (String, optional) the data format, blob or text
 * [return] data (Promise -> Object or Error)
 */
BzDeck.models.Bug.prototype.get_attachment_data = function (id, format = 'blob') {
  let attachment = this.attachments.find(att => att.id === id),
      index = this.attachments.findIndex(att => att.id === id);

  let convert = () => {
    let binary = window.atob(attachment.data);

    if ((attachment.is_patch || attachment.content_type.startsWith('text/')) && format === 'text') {
      return binary;
    }

    return new Blob([new Uint8Array([...binary].map((x, i) => binary.charCodeAt(i)))],
                    { 'type': attachment.content_type });
  };

  if (!attachment) {
    return Promise.reject(new Error(`The attachment ${id} cannot be found on this bug.`));
  }

  if (attachment.data) {
    return Promise.resolve({ [format]: convert(), attachment });
  }

  return BzDeck.controllers.global.request(`bug/attachment/${id}`,
                                           new URLSearchParams('include_fields=data')).then(result => {
    let att = result.attachments[attachment.id];

    if (!att || !att.data) {
      return Promise.reject();
    }

    this.attachments[index].data = attachment.data = att.data;
    this.merge(this.attachments); // Save

    return Promise.resolve({ [format]: convert(), attachment });
  }).catch(error => {
    return Promise.reject(new Error(`The attachment ${id} cannot be retrieved from Bugzilla.`));
  });
};
