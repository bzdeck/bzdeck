 ...
 */
BzDeck.views.BugTimelineEntry.prototype.create = function () {
...
BzDeck.views.BugTimelineEntry.prototype.create_history_entries = function () {
  ...
  return Promise.all(changes.map(change => {
    return this.create_history_entry(changer_name, time, change, comment).then($change => $fragment.appendChild($change));
  })).then(() => {
    return $fragment;
  });
};
